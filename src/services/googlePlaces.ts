...
const useGooglePlaces = () => {
    const [placesState, setPlacesState] = useState({ placeRef: null as any, error: false })
    ...
    useEffect(() => {
        loader.load().then((google) => {
            ...
            setPlacesState((prevState) => {
                return { ...prevState, placeRef: placeRef }
            })
        }).catch(error => {
...
const findPlace = ({placeId, placeRef, callbackFunction,
}:{placeId: string, placeRef: any,callbackFunction: (results: any, status: any) => void,
}) => {
